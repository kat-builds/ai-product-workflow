...
A validated PRD may hand off to `$generate-tasks` only when `references/handoff.md` permits it. Let `$generate-tasks` own its derived Task documents and rules.
...
8. Read `assets/prd-template.md` when assembling the document. Remove instructions, examples, placeholders, and untriggered conditional subsections from the final PRD.
...
12. If Task synchronization may be appropriate, read `references/handoff.md`. Continue with `$generate-tasks` only when that handoff rule permits it; otherwise stop after the validated PRD.
